...
func _setup() -> void:
	...
	_ui_reception_menu.visible = false # Hide dialogue at the start
...
func _on_stressed_pressed() -> void:
	...
	_dialogue_step = 10 # 10 means Stressed Path Step 1
	_update_dialogue_ui()

func _on_confident_pressed() -> void:
	_ui_reception_menu.visible = false
	_ui_dialogue_box.visible = true
	_dialogue_step = 20 # 20 means Confident Path Step 1
	_update_dialogue_ui()
...
func _on_dialog_choice_1_pressed() -> void:
	...
	if _dialogue_step == 10 or _dialogue_step == 20:
		_dialogue_step += 1 # Move to part 2 (11 or 21)
		_update_dialogue_ui()
	elif _dialogue_step == 11 or _dialogue_step == 21:
		print("SUCCESS! Player knows to go to the 2nd Floor!")
		_ui_dialogue_box.visible = false
		_change_substate(SubState.RESOLVE) # Or whatever room is next!

func _on_dialog_choice_2_pressed() -> void:
	# Choice 2 is the WRONG choice
	print("MISSION FAILED: The receptionist called security!")
	_ui_dialogue_box.visible = false
	# You can send them back to the start or handle failure here!
	...
	if _initialised:
		match _current:
			...
			SubState.INSIDE:
				_world_inside.visible = false
				_world_inside.process_mode = Node.PROCESS_MODE_DISABLED
				_ui_reception_menu.visible = false # Safety hide
			SubState.RESOLVE: 
				_ui_resolve.visible = false
