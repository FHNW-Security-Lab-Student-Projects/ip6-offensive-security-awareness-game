# Types a Label out character by character. The owner drives it: start(), then
# advance(delta) from _process, finish_now() on a click.
# Preload, no class_name: a bare `godot -s` run has no global class cache.
extends RefCounted

signal finished

# Matches DialogBox, so both screens read at the same speed.
...
# A new line while one runs simply replaces it; the caller sequences.
func start(label: Label, text: String, speed: float = DEFAULT_SPEED) -> void:
	...
	if label == null:
		...
		return
	label.text = text
	# visible_characters, not slicing: the label keeps its full layout from frame
	# one, so the box does not grow and nothing below it jumps.
